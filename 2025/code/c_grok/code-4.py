...
X_q1 = male_data_q1[["孕周", "孕妇BMI"]].assign(孕周2=lambda x: x["孕周"] ** 2)
...
male_data_q2["BMI_Group"] = pd.qcut(
    male_data_q2["孕妇BMI"], q=4, labels=["G1", "G2", "G3", "G4"], duplicates="drop"
)
...
male_data_q3["BMI_Group"] = pd.qcut(
    male_data_q3["孕妇BMI"], q=4, labels=["G1", "G2", "G3", "G4"], duplicates="drop"
)
...
y_q4 = (female_data_q4["染色体的非整倍体"] != "").astype(int)  # 需确认空值代表正常
X_q4 = female_data_q4[
    ["13号染色体的Z值", "18号染色体的Z值", "21号染色体的Z值", "X染色体的Z值", "孕妇BMI"]
]
...
print(f"女胎数据类分布: {np.bincount(y_q4)}")
if len(np.unique(y_q4)) < 2:
    # 若仅一类，使用Isolation Forest
    iso_forest = IsolationForest(contamination=0.1, random_state=42)
    iso_forest.fit(X_q4)
    anomaly_score = -iso_forest.score_samples(X_q4)
    y_pred_q4 = (anomaly_score > np.percentile(anomaly_score, 10)).astype(int)
    accuracy = np.mean(y_pred_q4 == 1)  # 单类下近似准确率
    auc = np.nan  # 明确标记AUC无效
else:
    lr_model = LogisticRegression(random_state=42)
    lr_model.fit(X_q4, y_q4)
    y_pred_q4 = lr_model.predict(X_q4)
...
# 可视化
plt.figure(figsize=(10, 6))
sns.scatterplot(x=anomaly_score, y=y_q4, hue=y_pred_q4, palette="viridis")
plt.title("女胎异常判定得分分布")
plt.xlabel("异常得分")
plt.ylabel("真实标签")
plt.savefig("anomaly_score_female.png")
...
print(f"准确率: {accuracy:.2f}, AUC: {auc}")
